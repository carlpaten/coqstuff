...
Ltac forSome f ls :=
  match ls with
    | tt => f ls
    | (?X, ?LS) => f X || forSome f LS || fail 1
  end.

Goal True.
  Fail forSome ltac:(fun x => exact x) (0, tt).
...
Goal True.
  pose 0 as A.
  Fail forSomeInCtx ltac:(fun x => exact x).
  pose I as B.
...
Goal True.
  Fail forEach ltac:(fun x => exact x) (0, (I, tt)).
...
Ltac invert1 H := inversion H; [idtac]; clear H; try subst.
...
Ltac subst1 := try subst.

Goal forall (x y: nat), x = y -> x = y.
  intros.
  progress subst1.
  Fail progress subst1. (* infaillible *)
  subst1. (* no-op *)
  exact eq_refl.
Qed.
