...
Proof.
  ...
  (* autorewrite ... using (x) applies tactic x after every rewrite *)
  apply T_Var; autorewrite with (* ENTER YOUR HintDb HERE *) using (auto; try omega).
Qed.
...
Lemma nth_last : forall A (ls : list A) x y,
  nth (length ls) (ls ++ x :: nil) y = x.
Proof.
  induction ls; auto.
Qed.
...
Proof.
  ...
  gen_subst; autorewrite with app in *; auto.
  (* Exercise: Why do we need autorewrite... twice here? *)
  apply T_Var; autorewrite with app in *; autorewrite with app in *; simpl in *;
    auto.
Qed.
