...
(* In the following definition, I'm somewhat disturbed by the need to supply
   UnitT as a default to nth. I tried to build this proof using a function
   nth_in_bounds, which took a proof that its n parameter is appropriate; 
   it then didn't need a default. However, the proofs became *ugly*, and it
   was a struggle to automate the process. A suggestion for a better formulation
   is welcome. *)
...
Hint Extern 1 (_ < length _) => simpl.
...
Proof.
  ...
  Hint Extern 1 (exists _ : _, eval (App ?e1 ?e2) _) =>
  match goal with
    | [ Hval1 : value e1
      , Hval2 : value e2
      , Htype : has_type _ e1 (Arrow _ _)
      |- _ ] => let Hlam := fresh in
                let Hrewrite := fresh in
                pose proof (canonical_form_lam Htype Hval1) as Hlam;
                inversion Hlam as [? Hrewrite];
                rewrite Hrewrite
  end.

  Hint Extern 2 =>
  match goal with
    | [ H : member_n _ _ nil |- _ ] => inversion H
  end.
...
Hint Resolve member_n_tail_switch.
...
Proof.
  Hint Resolve context_bounds_term app_nil_l.
  Hint Extern 2 => reflexivity.
...
Proof.
  Hint Resolve closed_term_weakening app_nil_r.
  Hint Extern 2 => solve [omega | congruence].
  Hint Extern 2 (has_type nil _ _) => gen_subst; erewrite member_n_last.
  Hint Extern 2 (_ < _) => match goal with
                             | [ H : member_n _ _ _ |- _ ] =>
                                 apply member_n_length in H;
